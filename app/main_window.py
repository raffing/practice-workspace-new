...
class PracticeWorkspace(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.tree_widget.setHeaderLabels(["Pratiche e Task"])
    ...
    def _load_document(self, file_path: Path):
        try:
            ...
            self._do_refresh_tree()
            self.workspace_label.setText(f"Documento: {file_path.name}")
    ...
    def _save_document(self):
        ...
        try:
            ...
            for line in lines:
                if line.startswith('# '):
                    name = line[2:].strip()
                    # 1. Se già nel dizionario, usa quello
                    if name in self.practice_paths:
                        rel_path = self.practice_paths[name]
                        new_lines.append(f'# [{name}]({rel_path})')
                    else:
                        # 2. Cerca la cartella nel workspace
                        folder = None
                        direct_path = self.workspace_path / name
                        if direct_path.is_dir():
                            folder = direct_path
                        else:
                            # Cerca ricorsivamente
                            for p in self.workspace_path.rglob(name):
                                if p.is_dir():
                                    folder = p
                                    break
                        ...
                        if folder:
                            try:
                                rel = folder.relative_to(self.workspace_path)
                                rel_str = str(rel).replace('\\', '/')
                                self.practice_paths[name] = rel_str
                                new_lines.append(f'# [{name}]({rel_str})')
                            except ValueError:
                                new_lines.append(line)
                        else:
                            # Nessuna cartella trovata, lascia senza path
                            new_lines.append(line)
                else:
                    new_lines.append(line)
    ...
    def _open_practice_folder(self, practice_name: str) -> bool:
        ...
        if practice_name in self.practice_paths:
            ...
            if full_path.is_dir():
                ...
                self.status_bar.showMessage(f"Aperta cartella: {practice_name}", 3000)
                return True
        direct_path = self.workspace_path / practice_name
        if direct_path.is_dir():
            try:
                rel = direct_path.relative_to(self.workspace_path)
                self.practice_paths[practice_name] = str(rel).replace('\\', '/')
            except ValueError:
                pass
            os.startfile(str(direct_path))
            self.status_bar.showMessage(f"Aperta cartella: {practice_name}", 3000)
            return True
        for path in self.workspace_path.rglob(practice_name):
            if path.is_dir():
                try:
                ...
                self.status_bar.showMessage(f"Aperta cartella: {practice_name}", 3000)
                return True
        self.status_bar.showMessage(f"Cartella non trovata per '{practice_name}'", 3000)
        return False
    ...
    def _do_refresh_tree(self):
        ...
        for practice_node in nodes:
            ...
            tooltip += f"\nLinea: {practice_node.line_number + 1}"
            practice_item.setToolTip(0, tooltip)
            font = practice_item.font(0)
            font.setBold(True)
            practice_item.setFont(0, font)
            ...
            self.tree_widget.addTopLevelItem(practice_item)
    ...
    def _insert_autocomplete_item(self, item: str, position: int, full_path: Path = None):
        ...
        if item == "➕ Nuova pratica":
            ...
            if is_practice and full_path is not None:
                # Percorso deterministico dal popup
                if full_path.is_dir():
                    try:
                        ...
                        pass
                cursor.insertText(f" {item}\n")  # <-- Aggiunto lo spazio prima di item
            else:
    ...
    def _open_practice_folder(self, practice_name: str) -> bool:
        """Apre la cartella SOLO se ha un path deterministico."""
        if not self.workspace_path:
            return False
    ...
    def _save_document(self):
        """Salva SOLO con path deterministici dal dizionario."""
        if not self.workspace_path:
            return
        agenda_file = self.workspace_path / "Agenda.md"
        try:
    ...
    def _create_new_practice(self, name: str):
        ...
        self._save_document()
        self.status_bar.showMessage(f"Pratica '{name}' creata con path: {self.practice_paths.get(name, 'senza path')}", 3000)
        self._do_refresh_tree()
    ...
    def _open_file(self, filename: str):
        """Apre il file SOLO nella cartella della pratica corrente."""
        if not self.workspace_path:
            return
        ...
        # Cerca nei sottodirectory della pratica
    ...
    def _on_tree_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        if item_type == 'file':
            ...
            if filepath and Path(filepath).exists():
                ...
                self._open_file(data.get('filename'))
            return
    ...
    def _apply_filter(self):
        ...
        def filter_items(item: QTreeWidgetItem):
            ...
            if filter_text and filter_text not in text:
                visible = False
            if show_open_only and '☑' in item.text(0):
                visible = False
            if show_today_only and today_str not in text and '📅' in text:
                visible = False
            if show_with_files:
                has_files = False
                for i in range(item.childCount()):
                    ...
                    if child_data and child_data.get('type') in ('file', 'file_missing'):
                        has_files = True
                        break
                if not has_files and '📎' not in text:
                    visible = False
    ...
    def closeEvent(self, event):
        ...
        if self.workspace_path and self.editor.toPlainText() != self.editor.last_saved_content:
            reply = QMessageBox.question(
