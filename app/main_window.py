...
class PracticeWorkspace(QMainWindow):
    """Finestra principale dell'applicazione Practice Workspace."""
    ...
    def __init__(self):
        ...
        self._autosave_interval = 60  # seconds, 0 = disabled
    ...
    def _create_left_panel(self) -> QWidget:
        """Crea il pannello sinistro con barra di ricerca e albero."""
        left_panel = QWidget()
    ...
    def _setup_menus(self):
        """Configura i menu dell'applicazione."""
        # File menu
        ...
        settings_action.triggered.connect(self.settings_manager.show_dialog)
    ...
    def _setup_connections(self):
        """Configura i collegamenti tra i segnali e gli slot."""
        # Menu actions
    ...
    def _setup_shortcuts(self):
        ...
        shortcuts = [
            ("Ctrl+P", self._show_command_palette),
            ("Ctrl+Shift+H", self._show_history),
            ("Ctrl+Shift+L", self._format_document),
            ("Ctrl+T", self._toggle_task_in_corso),
        ]
    ...
    def _open_workspace(self):
        """Apre un workspace selezionato dall'utente."""
    ...
    def _load_workspace(self):
        ...
        # Create default agenda if doesn't exist
        agenda_file = self.workspace_path / "Agenda.md"
        if not agenda_file.exists():
            self._create_default_agenda(agenda_file)
    ...
    def _create_default_agenda(self, agenda_file: Path):
        """Crea un file Agenda.md di default."""
        default_content = (
            "# Villa Rossi\n"
        ...
        agenda_file.write_text(default_content, encoding='utf-8')
    ...
    def _load_document(self, file_path: Path):
        """Carica il documento dall'Agenda.md."""
        try:
    ...
    def _save_document(self, is_autosave: bool = False):
        ...
        try:
            ...
            if (self.document_history and 
                self.editor.last_saved_content and 
                content_with_paths != self.editor.last_saved_content):
    ...
    def _on_document_changed(self):
        """Gestisce le modifiche al documento."""
        self._update_modified_indicator()
    ...
    def _update_modified_indicator(self):
        """Aggiorna l'indicatore di modifiche nella status bar."""
        if not self.workspace_path:
            return
    ...
    def _do_refresh_tree(self):
        """Aggiorna l'albero del workspace."""
        self.tree_widget.clear()
        if not self.workspace_path:
            return
        
        # Root workspace
        self._add_root_workspace()
        
        # Practices from Agenda
        text = self.editor.toPlainText()
        if text:
            self._add_practices_from_agenda(text)

    def _add_root_workspace(self):
        """Aggiunge il nodo radice del workspace all'albero."""
    ...
    def _add_practices_from_agenda(self, text: str):
        """Aggiunge le pratiche dall'Agenda all'albero."""
        nodes = self.parser.parse_document(text)
        
        practices_item = self._create_practices_header()
        ...
        for practice_node in nodes:
            ...
            if practice_files:
                self._add_files_section(practice_item, practice_files)
            
            practice_item.setExpanded(True)
        
        practices_item.setExpanded(True)

    def _create_practices_header(self) -> QTreeWidgetItem:
        """Crea l'header della sezione pratiche."""
        item = QTreeWidgetItem()
        item.setText(0, "📋 Pratiche in Agenda")
        item.setForeground(0, QColor(self.current_theme['accent']))
        font = item.font(0)
        font.setBold(True)
        item.setFont(0, font)
    ...
    def _create_practice_item(self, practice_node: DocumentNode) -> QTreeWidgetItem:
        """Crea un item dell'albero per una pratica."""
        item = QTreeWidgetItem()
        item.setText(0, f"📁 {practice_node.text}")
        ...
        item.setForeground(0, QColor(self.current_theme['tree_practice']))
        return item

    def _add_files_section(self, parent_item: QTreeWidgetItem, files: set):
        """Aggiunge la sezione file collegati a una pratica."""
        file_section = QTreeWidgetItem()
        file_section.setText(0, f"📎 File collegati ({len(files)})")
        ...
        for filename in sorted(files):
            file_item = self._create_file_item(filename, 
                parent_item.data(0, Qt.ItemDataRole.UserRole)['text'])
            if file_item:
                file_section.addChild(file_item)

    def _populate_tree_folder(self, parent_item: QTreeWidgetItem, path: Path, depth: int = 0):
        """Popola ricorsivamente l'albero con il contenuto delle cartelle."""
        if depth > 3:
            return
        ...
        try:
            ...
            for item in items:
                if item.name.startswith('.') or item.name == 'Agenda.md':
                    continue
    ...
    def _add_task_to_tree(self, task_node: DocumentNode, parent_item: QTreeWidgetItem, 
                         practice_files: set):
        """Aggiunge un task all'albero ricorsivamente."""
        ...
        # Costruisci il testo visualizzato
        display_text = f"{icon} {clean_text}"
        extras = self._get_task_extras(task_node.task_data)
        if extras:
    ...
    def _get_task_extras(self, task_data: Optional[Dict]) -> list:
        ...
        if task_data.get('tags'):
            extras.append(' '.join(f'#{t}' for t in task_data['tags']))
        if task_data.get('people'):
            extras.append(' '.join(f'@{p}' for p in task_data['people']))
        if task_data.get('dates'):
            extras.append(f"📅 {', '.join(task_data['dates'])}")
        
        return extras

    def _create_file_item(self, filename: str, practice_name: str) -> Optional[QTreeWidgetItem]:
        """Crea un item per un file collegato."""
        if not self.workspace_path:
            return None
    ...
    def _make_missing_file_item(self, filename: str, reason: str) -> QTreeWidgetItem:
        """Crea un item per un file mancante."""
        item = QTreeWidgetItem()
    ...
    def _on_tree_item_clicked(self, item: QTreeWidgetItem, column: int):
        ...
        elif item_type == 'file_missing':
            QMessageBox.warning(self, "File non trovato",
                               f"Il file '{data.get('filename')}' non è stato trovato.")
        elif data.get('line_number') is not None:
            self._navigate_to_line(data['line_number'])

    def _on_tree_context_menu(self, pos):
        """Gestisce il menu contestuale dell'albero."""
        ...
        if data.get('type') == 'folder' and not data.get('is_practice'):
            ...
            if action == add_action:
                self._add_folder_as_practice(Path(data['path']))
    ...
    def _apply_filter(self):
        """Applica il filtro di ricerca all'albero."""
    ...
    # ========================================================================
    # FILE & PRACTICE NAVIGATION
    # ========================================================================

    def _open_path(self, path: Path):
        """Apre un file o una cartella in modo portabile."""
    ...
    def _open_practice_folder(self, practice_name: str) -> bool:
        """Apre la cartella di una pratica."""
        if not self.workspace_path:
            return False
    ...
    def _navigate_to_line(self, line_number: int):
        """Naviga a una linea specifica nell'editor."""
    ...
    def _show_autocomplete(self, type: str, position: int, practice_name: str = ""):
        """Mostra il popup di autocompletamento."""
        if not self.workspace_path:
            return
        ...
        if type == 'file' and practice_name in self.practice_paths:
            practice_dir = self.workspace_path / self.practice_paths[practice_name]
    ...
    def _on_autocomplete_closed(self):
        """Gestisce la chiusura del popup di autocompletamento."""
    ...
    def _new_practice_dialog(self):
        """Mostra il dialogo per creare una nuova pratica."""
        name, ok = QInputDialog.getText(self, "Nuova Pratica", "Nome pratica:")
        if ok and name:
            self._create_new_practice(name)

    def _create_new_practice(self, name: str):
        """Crea una nuova pratica con la struttura standard."""
        if not self.workspace_path:
            return
    ...
    def _on_practice_renamed(self, old_name, new_name):
        ...
        if not new_name or new_name in self.practice_paths:
            return
    ...
    def _show_command_palette(self):
        """Mostra la palette dei comandi."""
        if self.command_palette:
            self.command_palette.close()
        
        self.command_palette = CommandPalette(self, self.current_theme)
        self.command_palette.commandExecuted.connect(self.command_handler.execute)
        self.command_palette.show_centered()

    def _on_command_executed(self, command_id: str, param: str):
        """Esegue un comando dalla palette."""
        commands = {
            "save": self._save_document,
            "refresh": self._do_refresh_tree,
            "focus_editor": self.editor.setFocus,
            "new_practice": self._new_practice_dialog,
        }
    ...
    def _show_history(self):
        ...
        from PySide6.QtWidgets import QDialog, QListWidget, QListWidgetItem, QTextEdit, QVBoxLayout
        ...
        for i, entry in enumerate(reversed(entries)):
            ...
            item.setData(Qt.ItemDataRole.UserRole, start_index + len(entries) - 1 - i)  # CORRETTO
        ...
        def show_diff(current, previous):
            if not current:
                diff_view.clear()
                return
            index = current.data(Qt.ItemDataRole.UserRole)  # CORRETTO
    ...
    def _update_presence(self):
        """Aggiorna la presenza nel workspace condiviso."""
        if self.presence:
            self.presence.touch_active()

    def _check_presence(self):
        """Controlla la presenza di altri utenti."""
        if self.presence:
    ...
    def _on_file_changed(self, path: str):
        """Gestisce le modifiche esterne al file."""
